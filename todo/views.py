    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todo/signupuser.html', {'form': UserCreationForm, 'error': 'Ошибка создания пользователя'})
        else:
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todo/loginuser.html', {'form': AuthenticationForm, 'error': 'Ошибка авторизации'})
        else:
            login(request, user)
            return redirect('currenttodos')
